...
pub const Client = struct {
    ...
    pub fn run(self: *Client) Server.Error!void {
        ...
        const helloMessage = Hello{ .language = "abstractlanguage" };

        try self.write(Hello, helloMessage);
        ...
        for (DECLARATIONS, 0..) |rawDecl_, index| {
            const rawDecl = RawDeclaration{
                ...
                .ty = rawDecl_.ty,
                .index = index,
                .last = index == DECLARATIONS.len - 1,
            };
            try self.write(RawDeclaration, rawDecl);
            ...
            while (true) {
                ...
                switch (message) {
                    .question => {
                        const answer = SubtypeAnswer{
                            .isChild = defaultSubtype(message.question.parent, message.question.child),
                        };
                        _ = try self.write(SubtypeAnswer, answer);
                    },
                    .status => {
                    ...
                    else => return Server.Error.UnsupportedOperation,
                }
            }
        }
    }
